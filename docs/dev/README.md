...
- **[ACTUAL-STARTUP-FLOW.md](ACTUAL-STARTUP-FLOW.md)**: **起動の真の姿**。Windows はネイティブ・Mac/Linux は Docker 依存など、スクリプトを追った実際のフロー（ドキュメントと食い違うときはこちらを正とする）。
...
| | [ACTUAL-STARTUP-FLOW.md](ACTUAL-STARTUP-FLOW.md) | 実際の起動フロー（Win=ネイティブ, Mac/Linux=Docker） |
