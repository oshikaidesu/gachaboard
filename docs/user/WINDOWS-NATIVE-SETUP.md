...
**start.bat をダブルクリック** → 1（ローカル）または 2（Tailscale）を選択するだけで起動できます。Postgres / MinIO / sync-server は自動でセットアップされ、Next.js も続けて起動します。
...
PowerShell で以下を実行すると、サインイン時に start.bat 2（Tailscale）が自動実行されます。
