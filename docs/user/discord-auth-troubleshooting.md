...
- **Windows**: `scripts/entry/start.bat` → メニューで **2（Localhost production）** または **1（Tailscale production）** など、依存サービスを含む項目を選択。PostgreSQL・MinIO・sync-server が自動で起動します。
- **Mac / Linux**: `./scripts/entry/start.sh` または `scripts/entry/start.command` を実行。同じく Postgres・MinIO・sync-server が起動します。

開発モードで Next.js だけ先に起動している場合は、別ターミナルで上記のいずれかを実行するか、`scripts/entry/start.bat` / `scripts/entry/start.sh` を一度実行してから `npm run dev` してください。PostgreSQL が起動したら `pg_isready -h localhost -p 18581`（Mac/Linux）で確認できます。
...
| チェック項目 | 確認方法 |
|---|---|
| PostgreSQL が起動しているか | `scripts/entry/start.bat` / `scripts/entry/start.sh` を実行したあと、`pg_isready -h localhost -p 18581`（Mac/Linux）やターミナル表示で確認 |
...
#    Windows: scripts/entry/start.bat → メニューでモード選択（ローカル開発なら 5 など）
#    Mac/Linux: ./scripts/entry/start.sh または scripts/entry/start.command
...
- **長時間スリープ後**: ホストがスリープすると Postgres 等も止まる。再度 `scripts/entry/start.bat` / `scripts/entry/start.sh` で起動する
