...
`postgres`、`sync-server`、`minio` が `Up` または `healthy` になっていれば正常です。
...
初回のみ、事前に `npm install --legacy-peer-deps`、`npx prisma generate`、`npx prisma db push` を実行してください。
...
詳細は [ENV-AND-DEPLOYMENT-MODES.md](ENV-AND-DEPLOYMENT-MODES.md) を参照してください。
...
動画・音声の変換に必要です。未インストールの場合、変換が失敗します。
...
| 症状 | 対処 |
|------|------|
| Discord ログイン後にエラー（Callback） | `npm run status` で PostgreSQL を確認してください。[discord-auth-troubleshooting.md](discord-auth-troubleshooting.md) も参照してください。 |
