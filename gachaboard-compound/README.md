...
※ スクリーンショットの再生成: `cd nextjs-web && npm run screenshots:all` で一括実行（シード → サーバ起動 → 撮影）。手動で行う場合は `npm run seed:e2e` → 別ターミナルで `npm run e2e:server` → `npm run screenshots`。OGP 用画像（`public/ogp.png`）も同時に生成される。
