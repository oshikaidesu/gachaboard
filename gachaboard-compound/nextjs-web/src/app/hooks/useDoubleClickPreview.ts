...
import { Editor, StateNode, type TLClickEventInfo, type TLImageShape, type TLImageAsset } from "@cmpd/compound";
...
export function useDoubleClickPreview(
  ...
  const registerHandler = useCallback(
    (editor: Editor) => {
      const smartHandIdleState =
        editor.getStateDescendant<IdleStateNode>("smart-hand.idle") ??
        editor.getStateDescendant<IdleStateNode>("select.idle");
      ...
      smartHandIdleState.onDoubleClick = function (info) {
        ...
        if (info.target === "shape") {
          ...
          if (shape.type === "image") {
            ...
            const aid = imageShape.props.assetId;
            if (!aid) return originalOnDoubleClick?.(info);
            const tlAsset = editor.getAsset(aid) as TLImageAsset | undefined;
            if (tlAsset?.props.src) {
              ...
              if (id) {
                onOpen({
                  ...
                  kind: "image",
                  sizeBytes: String((tlAsset.props as { fileSize?: number }).fileSize ?? 0),
                  lastKnownX: null,
                  lastKnownY: null,
                });
                return;
