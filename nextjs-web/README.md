...
- **環境変数**: リポジトリルートで `npm run setup:env`（`.env.example` → `nextjs-web/.env.local`）または `cp env.local.template .env.local`（このディレクトリ内）
...
すでに `scripts/entry/start.bat` / `scripts/entry/start.sh` で依存サービスが起動しているときは、nextjs-web だけで:
...
依存サービスをまだ起動していない場合は、先にリポジトリルートで `scripts/entry/start.bat`（Windows）または `./scripts/entry/start.sh`（Mac/Linux）を実行してください。
...
| `NEXTAUTH_URL` | ベース URL。未設定可（リクエスト Host から解決）。例: ローカル `http://localhost:18580`、Tailscale は `https://<マシン名>.ts.net`（Serve/スクリプトが整える。`PORT` を変えたらそれに合わせる） |
