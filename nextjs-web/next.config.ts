...
// ルートと nextjs-web の二重 package-lock があると、Next が workspace root を誤推論する。
// 本番の output トレースではリポジトリルートを基準にする（警告抑制・standalone 取り込み）。
// dev / Turbopack では outputFileTracingRoot を nextjs-web に限定しないと、
// @tailwindcss/postcss がルートの package.json から tailwindcss を解決しようとして失敗する。
const nextjsWebRoot = path.dirname(fileURLToPath(import.meta.url));
const monorepoRoot = path.resolve(nextjsWebRoot, "..");

export default function createNextConfig(phase: string): NextConfig {
  const outputFileTracingRoot =
    phase === PHASE_DEVELOPMENT_SERVER ? nextjsWebRoot : monorepoRoot;
