...
| **ShapeConnectHandles** | `ShapeConnectHandles.tsx` | シェイプのコネクト用ハンドル（四辺の三角から矢印で他シェイプと接続、コーナーの紫四角でリサイズ）。リサイズは **経路 A**（選択枠のハンドル）と **経路 B**（シェイプ上のコーナーハンドル）の二通り。ライブラリのコーナーは CSS で非表示にし、B を優先。 |
...
**リサイズの二経路**: 経路 A＝選択枠のエッジハンドル（SmartHandTool → `pointing_resize_handle` → 各 ShapeUtil の `onResize`）。経路 B＝シェイプ上の ShapeConnectHandles のコーナー（紫四角）。ライブラリのコーナーハンドルは `.tl-corner-handle { display: none }` で非表示にし、B を優先。GEO の「テキストがあるときはそれより小さくできない」は経路 A でライブラリの onResize により効く。
