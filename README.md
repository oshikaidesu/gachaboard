...
**Windows**: Windowsでは `start.bat` で起動します。
- **1** … ローカル（localhost）
...
Gachaboardは **Tailscale** を活用し、グローバル IP なしでも安全に外部公開できます。参加者は `start.bat` を実行する必要はなく、ブラウザでアクセスするだけで完了です。
...
| やること | 自動？ | 備考 |
|:---------|:------:|:-----|
| Node.js 18+ | 手動 | [nodejs.org](https://nodejs.org/) |
| PostgreSQL（システムに無い場合） | 手動 | Mac: `brew install postgresql@16` 等。スクリプトが案内を表示 |
...
| MinIO・sync-server・npm install・DB スキーマ | ✅ 自動 | 起動スクリプトが実行 |

### 起動コマンド一覧

| コマンド | 説明 |
|----------|------|
| **start.bat**（Windows） | 1=ローカル, 2=Tailscale, 3=リセット |
| **start.sh** / **start.command**（Mac/Linux） | Tailscale モードで起動（HTTPS） |
| `npm run dev` | 開発モード（Postgres / MinIO / sync-server は start スクリプトで起動済み前提） |
| `npm run dev:local` | 開発モード（localhost のみ） |
| `npm start` | ビルド済み起動（Tailscale） |
| `npm run reset:all` | **全データリセット**（DB / ストレージ削除） |
| `cd nextjs-web && npm run status` | DB / MinIO / sync-server の状態確認 |
...
| プラットフォーム | 事前準備 | 備考 |
|------------------|----------|------|
| **Windows** | Node.js のみ | **start.bat** で起動。PostgreSQL・MinIO・sync-server は自動セットアップ。詳細は [WINDOWS-NATIVE-SETUP.md](docs/user/WINDOWS-NATIVE-SETUP.md) |
| **Mac** | Node.js 18+（PostgreSQL は brew 等で別途、またはスクリプト案内に従う） | Tailscale 利用時: `brew install tailscale jq` |
| **Linux** | Node.js + PostgreSQL（またはスクリプト案内に従う） | Tailscale 利用時は tailscale / jq をインストール |
