...
**Windows (portable)**: Double-click **`scripts/entry/!-Gachaboard.bat`** (sorted first in that folder) to start **Gachaboard 0.1.0.exe** or **`Gachaboard.exe`** in the repo root. A built portable binary lives under `launcher/dist/`; if you only cloned the repo, grab the EXE from [Releases](https://github.com/oshikaidesu/gachaboard/releases) and place it at the repo root.
...
[Releases](https://github.com/oshikaidesu/gachaboard/releases) から **Gachaboard 0.1.0.exe**（portable）をダウンロードし、プロジェクトフォルダに配置して **exe をダブルクリック**で起動。初回はウィザードで Discord Client ID / Client Secret / オーナー ID を入力するだけで起動できます。ターミナルは出さず、トレイに常駐します。

```bash
git clone https://github.com/oshikaidesu/gachaboard.git
cd gachaboard
# Releases から Gachaboard 0.1.0.exe をダウンロードし、このフォルダに置いて exe を実行
```

### 軽量版・開発者向け（scripts/entry）

| OS | 起動方法 | 事前に必要なもの |
|:---|:---|:---|
| **Windows** | `scripts/entry/start.bat` をダブルクリック | [Node.js](https://nodejs.org/) |
| **Mac** | `scripts/entry/start.command` をダブルクリック | Node.js + [PostgreSQL](https://www.postgresql.org/) |
| **Linux** | `./scripts/entry/start.sh` | Node.js + PostgreSQL |
...
起動用 bat/sh は [`scripts/entry/`](scripts/entry/README.md) に集約されています。

### 友達を招待する

[Tailscale](https://tailscale.com/) を使えば、グローバル IP なしで他の端末からアクセスできます。参加者はブラウザでアクセスするだけです。

1. `scripts/entry/start.bat` → **1**（Tailscale 本番・既定）で起動
...
- [Windows 起動（scripts/entry/start.bat）](docs/user/WINDOWS-NATIVE-SETUP.md)
...
- **Tailscale 公開時**: [セキュリティ棚卸しチェックリスト](docs/user/SECURITY-AUDIT-CHECKLIST.md) で本番設定の確認ができます。
