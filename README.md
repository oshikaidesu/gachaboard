...
1. **タグを push** するか、**Actions → Release Build → Run workflow** で手動実行してください。
2. タグ（`v*`）を push した場合、Release が自動作成され、`gachaboard-dist.zip` が添付されます。
3. ダウンロードした zip を展開したのち、以下を実行してください。
   ```bash
   npm run setup:env
...
ビルド済み `.next` が含まれるため、`npm run build` は不要です。`node_modules` は `npm install` で取得してください。本番ビルドを手動で作成する手順は [docs/user/PRODUCTION-BUILD.md](docs/user/PRODUCTION-BUILD.md) を参照してください。
