...
**Mac / Windows (WSL2) / Linux** に対応しています。**Docker のみ**（Node.js 不要）または **Docker + Node.js** で起動できます。

### プラットフォーム別の起動方法

| OS | 起動方法 |
|:---|:---|
| **Mac** | `start.command` をダブルクリック（Tailscale モード・HTTPS） |
| **Windows** | `start.bat` をダブルクリック（WSL2 内で起動・Tailscale モード・HTTPS） |
| **Linux** | ターミナルで `bash scripts/start/tailscale.sh` または `npm run dev` |
...
| 方法 | 説明 |
|------|------|
| **Mac** | **`start.command` をダブルクリック**（Tailscale モード・HTTPS で起動） |
| **Windows** | **`start.bat` をダブルクリック**（WSL2 で起動・Tailscale モード・HTTPS） |
| ターミナル（Mac/WSL2/Linux） | `bash scripts/start/tailscale.sh` または `npm run dev` |
| ローカルのみ | `npm run dev:local` |
| 本番 | `cd nextjs-web && npm run build && cd ..` の後 `npm start` |

**Windows**: **WSL2 が必須**です。clone 後は **start.bat をダブルクリック**すると WSL2 が起動し、Tailscale モードで起動します。WSL2 未導入の場合は [docs/user/WSL2-SETUP.md](docs/user/WSL2-SETUP.md) を参照してください。プロジェクトは WSL2 内の `~/gachaboard` にクローンすることを推奨します。
...
| コマンド | 説明 |
|----------|------|
| **start.command**（Mac） | Tailscale モードで起動（HTTPS）。Node 未導入時は Docker のみ |
| **start.bat**（Windows） | WSL2 を起動し Tailscale モードで起動（HTTPS）。WSL2 必須 |
...
Tailscale モードでは **アクセスは HTTPS** になります（Tailscale Serve が証明書を提供）。Mac では `brew install tailscale jq` が必要です。Caddy で HTTPS を自前運用する場合は `npm run setup:tailscale-https` と [TAILSCALE_HTTPS_SETUP.md](docs/user/TAILSCALE_HTTPS_SETUP.md) を参照してください。
...
| プラットフォーム | 事前準備 | 備考 |
|------------------|----------|------|
| **Mac** | Node.js 18+、[Docker Desktop](https://www.docker.com/products/docker-desktop/) または Colima | Tailscale 利用時: `brew install tailscale jq`（**tailscale CLI** が必要） |
| **Windows** | [WSL2](https://docs.microsoft.com/ja-jp/windows/wsl/)（必須）、Docker Engine、Node.js、Tailscale（WSL2 内） | Docker Desktop 不要。WSL2 内で Docker Engine を直接使用。セットアップは [docs/user/WSL2-SETUP.md](docs/user/WSL2-SETUP.md) を参照 |
| **Linux** | `apt install docker.io docker-compose-v2 git nodejs npm` 等 | Tailscale 利用時: [tailscale.com/download](https://tailscale.com/download) で **tailscale CLI** をインストール |
...
- [WSL2 セットアップ（Windows）](docs/user/WSL2-SETUP.md)
