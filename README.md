...
**Windows（ポータブル）**: リポジトリ直下の **`!-Gachaboard.bat`**（一覧の先頭に表示されます）をダブルクリックすると、**Gachaboard 0.1.0.exe** または **Gachaboard.exe** を起動します。ビルド済みの portable は `launcher/dist/` にあります（clone しただけの環境では [Releases](https://github.com/oshikaidesu/gachaboard/releases) から exe を置いてください）。
