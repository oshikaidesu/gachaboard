...
function minioHealthUrl(s3Endpoint: string): string {
  try {
    ...
    return "http://localhost:18583/minio/health/live";
  }
}
...
async function main() {
  ...
  const nextUrl = (process.env.NEXTAUTH_URL || `http://localhost:${process.env.PORT || 18580}`).replace(/\/$/, "");
  const s3Endpoint = process.env.S3_ENDPOINT || "http://localhost:18583";
  ...
  if (!db.ok) {
    ...
    console.log("   → DATABASE_URL を .env.local で確認（例: localhost:18581）");
    console.log("");
    process.exit(1);
  }
