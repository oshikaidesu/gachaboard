...
ブラウザで表示された URL（例: `http://localhost:18580`）を開き、Discord でログイン。

> **Discord Redirect**: Redirects に `http://localhost:18580/api/auth/callback/discord` を追加。詳細は [docs/user/SETUP.md](docs/user/SETUP.md)。
...
| コマンド | 説明 |
|----------|------|
| **start.command**（Mac） / **npm run dev** | 開発モード（Docker + Next.js まとめて起動） |
...
**本番ビルド**（`npm start` を使う前）: `cd nextjs-web && npm run build`。詳細は [docs/user/PRODUCTION-BUILD.md](docs/user/PRODUCTION-BUILD.md)。

Tailscale モードでは `brew install tailscale jq` が必要。HTTPS は `npm run setup:tailscale-https` でセットアップ可能。→ [TAILSCALE_HTTPS_SETUP.md](docs/user/TAILSCALE_HTTPS_SETUP.md)

**起動確認**: `cd nextjs-web && npm run status` で Docker / DB / MinIO / sync-server を一覧表示。
...
詳細は [docs/user/SETUP.md](docs/user/SETUP.md) を参照。

---

## 📦 GitHub でビルドごと配布

[GitHub Releases](https://docs.github.com/ja/repositories/releasing-projects-on-github) でビルド済みアプリを配布できます。

1. **タグを push** するか、**Actions → Release Build → Run workflow** で手動実行
2. タグ（`v*`）の場合は自動で Release が作成され、`gachaboard-dist.zip` が添付されます
3. ダウンロードした zip を展開後:
   ```bash
   npm run setup:env
   # nextjs-web/.env.local を編集（Discord OAuth 等）
   cd nextjs-web && npm install --legacy-peer-deps && npx prisma generate && npx prisma db push && cd ..
   # 起動: start.command をダブルクリック または npm run dev
   ```

※ ビルド済み `.next` が含まれるため、`npm run build` は不要です。`node_modules` のみ `npm install` で取得します。自前で本番ビルドを作る手順は [docs/user/PRODUCTION-BUILD.md](docs/user/PRODUCTION-BUILD.md)。
