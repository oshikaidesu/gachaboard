...
$TaskDescription = "Gachaboard をログオン時に起動（WSL2 + Tailscale）"
# 最小化で起動（ウィンドウは出るがタスクバーに最小化）
$Action = New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c start /min `"`" `"$StartBat`"" -WorkingDirectory $RootDir
...
try {
  ...
  Write-Host ""
  Write-Host "  ログオン時に start.bat が実行されます。" -ForegroundColor Gray
